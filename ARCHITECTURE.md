...
- after launching or resuming, the outer agent should perform a short launch sanity check: run `python3 /path/to/council-agent/scripts/codex_tui_supervisor.py status <task> --dir <target-repo> --sessions` for execution runs, or add `--planning --sessions` for planning runs; verify the expected role reached a normal Codex prompt or started writing artifacts; and clear local Codex update/auth/setup prompts before classifying the council run as failed
